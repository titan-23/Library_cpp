...
現行の `sa/**`、関連するTSPとクラスタリングの例、文書、可視化用の補助ツールを独立に再読した。下記の旧本文は5ファイルだけを対象とし、スクリプトも除外していたため、今回の確認範囲を表さない。実行はしていない。

### 正しさに関する問題

- `sa/sa.cpp:151-176,216-231`: `sa_multi_run` の `NUM_STARTS<=0` は、時間配分のゼロ除算、初期化されていない最良状態・最良結果の参照、または負値による不正処理を起こす。
- `sa/sa.cpp:343-380`: `replica_run(...,NUM_REPLICAS<=0)` は空の `vector` に対する `states[0]` / `scores[0]` の参照、または負値からの巨大な領域確保になる。
- `sa/sa_state.cpp:31-35,63-67` の配布雛形は `TYPE_CNT=0`、`changed.type` 未初期化、`modify` が空である。`sa.cpp:48,78-86,234,258-267,370-374,431-444` は `type` を統計配列の添字へ使うため、そのままでは未定義動作になる。雛形を最低1種類にし、実行側でも `type` の範囲を検査する。
- `tsp/examples/sa_tsp.cpp:78-90,217-221` は `N<=3` のとき `valid=false` だけを設定し、上記の型別統計が未初期化値を読む。
- `sa/sa.cpp:30-76,151-197,241-256,416-432` はNaNまたは正の無限大の時間制限を拒否せず、処理が止まらなくなるか、NaNを温度表の添字へ整数変換することになる。負の無限大は即時終了するため同じ問題ではない。`FIRST_PHASE_RATIO` の範囲と有限性も検査しない。
- `sa_run` と `sa_multi_run` は `END_TEMP<=0` を `1e-9` へ丸める一方、非正または非有限の `START_TEMP` と、NaNまたは正の無限大の `END_TEMP` は残る。`replica_run` の `:320-353,502-510` は開始温度と終了温度を丸めず、非正または非有限の値で比や交換確率が壊れる。
- `tsp/examples/sa_tsp.cpp:462-466` は `ll` の4辺距離差を `int diff_d` へ狭め、改善判定・距離更新を誤る。

### 文書との不一致

`titan_cpplib/ahc/README.md:47` と `tsp/tsp_design.md:427,456` は、状態の生成関数またはラムダ式を `sa_run`, `sa_multi_run`, `replica_run` へ渡せると説明する。しかし `sa/sa.cpp:29-30,133-156,319-327` に、その引数を受け取る関数定義はない。実装を追加するか、既定構築後に `init(seed)` を呼ぶ現在の公開関数に合わせて文書を直す。

`sa/how_to_use_sa.md:5,26-31,102-120` は複数開始点での実行を説明せず、「探索中にコピーしない」という記述を広く読める。しかし `sa_multi_run` は最良値を更新するたびに `State` をコピーする。単一開始点と複数開始点の所有コストを分けて説明する。

仕様確認事項として、`sa_multi_run` の第1段階は各開始点を終了温度まで冷却した後、第2段階で `FIRST_PHASE_RATIO` 時点の温度へ再加熱する。意図した再開始なのか、連続した冷却過程なのかを明記する。

### 定数倍高速化

- `sa.cpp:46-48,80,82,84-86,90` と複数開始点版 `:233-234,260,263,265-267,271` は、`verbose=false` でも表示専用の回数表を確保・更新する。`replica_run` にも `:370-374,436,443-444` の型別統計が残る。`State::modify` に渡す `iter` は探索入力なので維持し、表示専用値だけをコンパイル時または実行時の分岐で省く。
- 時刻と進捗の取得を16〜32反復ごとに間引く案は、高頻度の反復処理に効く可能性が高い。ただし間の反復では進捗と温度が同じ値になり、制限時間超過の幅も増える。同じ乱数種でも結果が変わる、高速化用の明示的な設定として扱う。

旧本文にある `replica_run` の `iter` の長さ、同じ乱数種の使用、外部から乱数種を指定できないという指摘は現行で修正済みである。以下の本文は上記の訂正を優先して読む。

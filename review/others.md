...
- **[注意] `read(0, ...)` は `<unistd.h>` の関数**だが include していない。`<bits/stdc++.h>` は unistd を保証しないため、環境によってはコンパイルできない。
- **[軽微]** `#pragma once` がない上に、グローバル変数 `scanner` と非 inline の自由関数 `read_int64()` をヘッダ相当のファイルに定義している。複数回 include や複数訳単位で ODR 違反になる(expander 経由の単一ファイル運用なら実害なし)。
...
- **[注意] `assert` を使うのに `<cassert>` を include していない**。vector と algorithm しか include しておらず、単体では通らない可能性がある。
- **[軽微]** `namespace titan23` の外にあり、`#pragma once` もない。特に `index` は POSIX の同名関数(strings.h)とグローバル名前空間で衝突し得る名前で、`using namespace std` と組み合わせた際の曖昧さの種になる。titan23 に入れるか名前を変えるのが安全である。

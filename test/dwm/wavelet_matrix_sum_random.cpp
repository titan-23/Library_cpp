...
using Matrix = titan23::WaveletMatrixSum<Key, Weight>;
...
void verify(const vector<Key> &keys, const vector<Weight> &weights, const int sigma, const uint64_t seed) {
    Matrix matrix(sigma, keys, weights);
    vector<Key> ordinary_keys = keys;
    titan23::WaveletMatrix<Key> ordinary(sigma, ordinary_keys);
    assert(matrix.len() == static_cast<int>(keys.size()));
    ...
    assert(matrix.toitems() == items);
    for (int i = 0; i < static_cast<int>(keys.size()); ++i) {
        assert(matrix.access(i) == keys[i]);
        assert(matrix.access_weight(i) == weights[i]);
        assert(matrix.access_pair(i) == items[i]);
        assert(matrix.access(i) == ordinary.access(i));
    }
    ...
    for (int query = 0; query < 10000; ++query) {
        ...
        assert(matrix.rank(r, value) == ordinary.rank(r, value));
        int range_count = 0;
        ...
        if (upper < sigma) assert(matrix.range_freq(l, r, upper) == ordinary.range_freq(l, r, upper));
        ...
        assert(matrix.kth_smallest(l, r, k) == ordinary.kth_smallest(l, r, k));
...
void test_random() {
    for (uint64_t seed = 0; seed < 12; ++seed) {
        ...
        verify(keys, weights, sigma, seed * 999983 + 17);
    }
}
    ...
    for (int l = 0; l <= static_cast<int>(keys.size()); ++l) {
        for (int r = l; r <= static_cast<int>(keys.size()); ++r) {
            ...
            for (int upper = 0; upper <= 4; ++upper) {
                ...
                assert(matrix.sum_lt(l, r, upper) == sum);
            }
            for (int k = 0; k <= r - l; ++k) {
                assert(matrix.sum_k_smallest(l, r, k) == naive_sum_k(keys, weights, l, r, k, false));
                assert(matrix.sum_k_largest(l, r, k) == naive_sum_k(keys, weights, l, r, k, true));
            }
        }
    }
}
...
void test_value_weight() {
    vector<Key> values = {7, 1, 4, 1, 9, 0, 9, 1};
    Matrix matrix(10, values);
    vector<Weight> weights(values.begin(), values.end());
    assert(matrix.tovector() == values);
    assert(matrix.toweights() == weights);
    for (int l = 0; l <= static_cast<int>(values.size()); ++l) {
        for (int r = l; r <= static_cast<int>(values.size()); ++r) {
            ...
            for (Weight target = 0; target <= total + 1; ++target) {
                assert(matrix.min_count_smallest_sum_ge(l, r, target) == naive_min_count(values, weights, l, r, target, false));
                assert(matrix.min_count_largest_sum_ge(l, r, target) == naive_min_count(values, weights, l, r, target, true));
            }
        }
    }
}
...
void test_large_sigma() {
    using LargeMatrix = titan23::WaveletMatrixSum<long long, long long>;
    ...
    assert(matrix.tovector() == keys);
    assert(matrix.toweights() == weights);
    assert(matrix.sum_k_largest(0, keys.size(), 3) == 9);
    assert(matrix.sum_lt(0, keys.size(), 1LL << 32) == 12);
}
...
int main() {
    ...
    test_empty();
    test_large_sigma();
    cout << "Wavelet matrix sum random test: OK" << '\n';
    return 0;
}

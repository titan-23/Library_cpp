// DyCone の正当性ストレステスト
// ランダムな操作列を生成し、毎クエリ DSU を作り直す愚直解と照合する
    ...
    for (int it = 0; it < iters; ++it) {
        ...
        for (int i = 0; i < q; ++i) {
            ...
            if (r < 25) {
                ...
                ops.push_back({3, (int)(rng() % n), 0, 0});
            } else if (r < 85) {
                ops.push_back({4, (int)(rng() % n), 0, 0});
            } else if (r < 95) {
                ops.push_back({5, (int)(rng() % n), (int)(rng() % n), 0});
            } else {
                ops.push_back({6, 0, 0, 0});
            }
        }

        // DyCone
        titan23::DyCone<ll> dc(n, rng());
        ...
        dc.run([&] (int k) {
            ...
            if (o.t == 3) got.push_back(dc.sum(o.u));
            else if (o.t == 4) got.push_back(dc.size(o.u));
            else if (o.t == 5) got.push_back(dc.same(o.u, o.v));
        ...
        if (got != want) {
            ...
            for (int i = 0; i < (int)got.size(); ++i) {
                if (got[i] != want[i]) {
                    cout << "  query " << i << ": got " << got[i] << ", want " << want[i] << "\n";
                }
            }
            return 1;
        }

// DyConeSum とセグ木 DFS 版 (OfflineDynamicConnectivitySum) の比較ベンチマーク
// 同一のランダム操作列を両者に流し、答えの一致検証と実行時間の計測を行う
//
// g++ -std=c++20 -O2 -Wall -Wextra -I . -o bench test/graph/dycone/bench.cpp
// ./bench [n] [q] [seed] [mode]   mode: 0=両方(検証つき) 1=DyConeSum のみ 2=セグ木 DFS のみ

#include <bits/stdc++.h>
#include <sys/resource.h>
#include "titan_cpplib/graph/dycone_sum.cpp"
#include "titan_cpplib/graph/offline_dynamic_connectivity_sum.cpp"
using namespace std;
using ll = long long;
    ...
    if (mode == 0) {
        if (a1 == a2) {
            ...
            for (int i = 0; i < (int)min(a1.size(), a2.size()); ++i) {
                if (a1[i] != a2[i] && bad++ < 5) {
                    cout << "mismatch at " << i << ": DyConeSum " << a1[i]
                         << ", SegDFS " << a2[i] << endl;
                }
            }

...
/// @brief cand から導出した frontier と隣接葉 LCP で候補間の状態差分を復元するビームサーチ
...
private:
    ...
    inline Slot frontier_action_slot(int ordinal) const {
        return cand[cand.size() - 1 - ordinal].action_slot;
    }
    ...
    /// @brief 親 slot を旧 cand から確定し、最終候補を世代別ブロックへ格納する
    void finalize_generation(int depth) {
        ...
        if (depth == 1) {
            parent_block[depth].assign(sz, BAD_SLOT);
        } else {
            for (int i = 0; i < sz; ++i) {
                int parent_leaf = candidates.next_beam[i].parent_leaf;
                parent_block[depth].push_back(frontier_action_slot(parent_leaf));
            }
        }
    ...
    /// @brief root 直下の候補から最初の隣接葉 LCP を作る
    ...
    /// @brief sort 後の候補から次世代の隣接葉 LCP を構築する
    void build_next_frontier(int depth) {
        int old_size = (int)adjacent_lcp.size() + 1;
...
public:
    ...
    Result search(BeamParam &param, const bool verbose=false, const string& history_file = "") {
        ...
        init_bs();
        if (verbose) {
            beam_log::start_banner(cerr, "BeamSearchWithTree (parent compact)", param);
        ...
        for (int turn = 1; turn < param.max_turn; ++turn) {
            ...
            for (int j = 0; j < (int)cand.size(); ++j) {
                ...
                Slot target_slot = c.action_slot;

    ...
    void init() {
        ...
        for (int k = 0; k < K; ++k) {
            ...
            for (int i = 0; i < (int)cluster_cities.size(); ++i) {
                int next_city_idx = -1;
                ScoreType min_d = LLONG_MAX;

                for (int j = 0; j < (int)cluster_cities.size(); ++j) {
                    if (visited[j]) continue;
                    ScoreType d = DIST[current_node][cluster_cities[j]];
                    if (d < min_d) {
                        min_d = d;
                        next_city_idx = j;
                    }
    ...
    void modify(const ScoreType threshold, const double progress) {
        ...
        if (changed.type >= 0 && changed.type <= 3) {
            ...
            if (changed.type == 0) { // 2-opt
                int u_idx = sarnd.randrange(sz - 1) + 1;
                int u_prev = u_idx - 1;
                ...
                if (v_idx == 0) { is_valid = false; return; }
                ...
                if (real_u <= 0) { is_valid = false; return; }
                changed.u_idx = real_u; changed.v_idx = real_v;
                int real_u_prev = real_u - 1, real_v_next = (real_v + 1) % sz;
                ScoreType removed_dist = DIST[tr[real_u_prev]][tr[real_u]] + DIST[tr[real_v]][tr[real_v_next]];
                ScoreType added_dist = DIST[tr[real_u_prev]][tr[real_v]] + DIST[tr[real_u]][tr[real_v_next]];
                ...
                int u_idx = sarnd.randrange(sz - 1) + 1;
                if (u_idx + L > sz) { is_valid = false; return; }
                ...
                ScoreType removed_dist = DIST[tr[u_prev]][tr[u_idx]] + DIST[tr[u_last]][tr[u_next]] + DIST[tr[v_idx]][tr[v_next]];
                ScoreType added_dist;
                if (changed.type == 1) {
                ...
                ScoreType removed_dist = DIST[tr[a1]][tr[a2]] + DIST[tr[b1]][tr[b2]]
                                + DIST[tr[c1]][tr[c2]] + DIST[tr[d1]][tr[d2]];
                ScoreType added_dist   = DIST[tr[a1]][tr[c2]] + DIST[tr[d1]][tr[b2]]
            ...
            ScoreType block_internal = 0;
            for (int i = u_idx; i < v_idx; ++i) {
                block_internal += DIST[tours[r1][i]][tours[r1][i + 1]];
            }
            ScoreType removed1 = DIST[tours[r1][u_prev]][u_node] + DIST[v_node][tours[r1][v_next]];
            ScoreType added1   = DIST[tours[r1][u_prev]][tours[r1][v_next]];
            changed.diff_d1 = added1 - removed1 - block_internal;
            ScoreType removed2 = DIST[w_node][w_next_node];
            ScoreType added2   = DIST[w_node][u_node] + DIST[v_node][w_next_node];
            changed.diff_d2 = added2 - removed2 + block_internal;
            ScoreType old_d1 = dists[r1], old_d2 = dists[r2];
            ...
            ScoreType block_internal1 = 0;
            for (int i = u1_idx; i < v1_idx; ++i) {
                block_internal1 += DIST[tours[r1][i]][tours[r1][i + 1]];
            }
            ScoreType block_internal2 = 0;
            for (int i = u2_idx; i < v2_idx; ++i) {
                block_internal2 += DIST[tours[r2][i]][tours[r2][i + 1]];
            }
            ScoreType removed1 = DIST[tours[r1][u1_prev]][u1] + DIST[v1][tours[r1][v1_next]];
            ScoreType removed2 = DIST[tours[r2][u2_prev]][u2] + DIST[v2][tours[r2][v2_next]];
            ScoreType added1 = DIST[tours[r1][u1_prev]][u2] + DIST[v2][tours[r1][v1_next]];
            ScoreType added2 = DIST[tours[r2][u2_prev]][u1] + DIST[v1][tours[r2][v2_next]];
...
void solve() {
    ...
    // OMP_NUM_THREADS=32 time ./a.out < rd400.tsp > out.txt

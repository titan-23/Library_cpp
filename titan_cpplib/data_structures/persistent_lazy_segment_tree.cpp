...
public:
    struct MemoeyAllocator {
        ...
        struct Data {
            T key, data; F lazy;;
            Data() {}
            Data(T k, T d, F l) : key(k), data(d), lazy(l) {}
        };
        // #pragma pack(pop)
        ...
        MemoeyAllocator() : ptr(1), cap(1) {
            ...
            data.emplace_back(T{}, T{}, F{});
        }

        int copy(int node) {
            int idx = new_node(data[node].key, data[node].lazy);
  ...
  public:
    ...
    PLSEG apply(int l, int r, F f) {
        ...
        auto dfs = [&] (auto &&dfs, int node, int left, int right) -> int {
            ...
            if (l <= mid && mid < r) ma.data[nnode].key = mapping(f, ma.data[nnode].key);
            if (ma.tree[nnode].right) ma.tree[nnode].right = dfs(dfs, ma.tree[nnode].right, mid+1, right);
            update(nnode);
            return nnode;
        };
    ...
    PLSEG copy_from(PLSEG &from, int l, int r) {
        ...
        auto dfs = [&] (auto &&dfs, int fr, int to, int left, int right) -> int {
            ...
            if (l <= mid && mid < r) {
                // lazy,revは伝播済み、data,sizeはこの後updateする

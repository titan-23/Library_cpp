/// https://github.com/titan-23/Library_cpp/blob/main/titan_cpplib/ds/persistent_seg_wbtree.cpp
#pragma once
...
public:
    struct MemoryAllocator {
        ...
        // #pragma pack(push, 1)
        struct Data {
            T key, data; int8_t rev;
            Data() : rev(0) {}
            Data(T k, T d, int8_t r) : key(k), data(d), rev(r) {}
        };
        // #pragma pack(pop)
        ...
        SizeType copy(SizeType node) {
            SizeType idx = new_node(data[node].key);
...
private:
    ...
    PersistentSegWBTree(SizeType root) : root(root) {}
  ...
  public:
    ...
    PLTM insert(SizeType k, T key) {
        ...
        SizeType new_node = ma.new_node(key);
    ...
    friend ostream& operator<<(ostream& os, PersistentSegWBTree<SizeType, T, op, e> &tree) {

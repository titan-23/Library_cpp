...
private:
    ...
    void _build(const vector<T> &keys, const vector<W> &weights) {
        ...
        for (int bit = _log - 1; bit >= 0; --bit) {
            ...
            for (int i = 0; i < _n; ++i) {
                ...
                if (b) {
                    ...
                    ++zeros;
                    zero_weights[i] = current_weights[i];
                }
            }
            _v[bit].build();
            _mid[bit] = zeros;
        ...
        _leaf_sum = FenwickTree<W>(current_weights);
    }
    ...
    int _leaf_min_count(const int l, const int r, const T key, const W target) const {
        ...
        const int position = _leaf_sum.bisect_left(_leaf_sum.pref(l) + target);
...
public:
    ...
    void add_weight(const int k, const W delta) {
        ...
        int position = k;
        for (int bit = _log - 1; bit >= 0; --bit) {
            const bool b = _v[bit].access(position);

...
void check_diagram(const VoronoiResult& result) {
    ...
    if (n > 0) {
        long double expected = (result.bounds.max_x - result.bounds.min_x) * (result.bounds.max_y - result.bounds.min_y);
...
void test_coordinate_getters() {
    ...
    auto result = voronoi_diagram(points, [](const auto& point) { return point.first; }, [](const auto& point) { return point.second; }, {-10, -10, 10, 10});
